...
def init():
    ...
    cursor = cnn.cursor();
    cursor.execute("create table if not exists My_Record(id integer primary key autoincrement,name text,price integer,category text)")
    cursor.close();
    cnn.close();
init();
...
def insert_data():
    ...
    if(a>0 and b>0):
        cnn = sq.connect("MY.db");
        cursor = cnn.cursor();
        cursor.execute("insert into My_Record(name,price,category)values(?,?,?)",(name,price,category));
        txt1.delete(0,END);
        txt2.delete(0,END);
        txt1.focus();
        cnn.commit();
...
def update_data():
    ...
    if (a>0):
        ...
        txt5.insert(0,v[2])
        lbl6 = Label(gp4,text="Category");
        lbl6.place(x=20,y=130);
        txt6 = Entry(gp4,textvariable=ca1);
        txt6.place(x=80,y = 180);
        ...
        def up_data():
            ...
            cnn = sq.connect("MY.db");
            cursor = cnn.cursor();
            cursor.execute("update My_Record set name=(?),price=(?),category(?) where id=(?)",(name,price,category,v[0]));
            cnn.commit();
...
gp1 = LabelFrame(root,text="Insert Records",width="400",height="250")
...
gp3 = LabelFrame(root,text="All Records",width="300",height="500")
gp3.pack(padx=200,pady=10,fill="both",expand=False);
...
def display():
    ...
    rows = cursor.fetchall();
    global data 
    data = ttk.Treeview(gp3,columns=["id","name","price","category"], show="tree headings")
    data.heading("id",text="id")
    data.heading("name",text="name")
    data.heading("price",text="price")
    a = {};
...
lbl4 = Label(gp1,text="Category");
lbl4.place(x=30,y=130);
txt4 = Entry(gp1,textvariable=ca);
txt4.place(x=83,y=130);
